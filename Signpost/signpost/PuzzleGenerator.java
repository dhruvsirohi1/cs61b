...
class PuzzleGenerator implements PuzzleSource {
    ...
    /** Returns the unique square in MODEL to which unconnected square START
    ...
    static Sq findUniqueSuccessor(Model model, Sq start) {
        ...
        } else if (start.sequenceNum() != model.width() * model.height()) {
            for (Place p : start.successors()) {
                if (start.connectable(model.get(p))) {
                    count++;
                    temp = model.get(p);
                }
                if (model.get(p).sequenceNum() == start.sequenceNum() + 1) {
                    return model.get(p);
                }
            }
            if (count == 1) {
