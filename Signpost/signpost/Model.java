...
class Model implements Iterable<Model.Sq> {
        ...
        for (int i = 0; i < _width; i += 1) {
            for (int j = 0; j < _height; j += 1) {
                if (_solution[i][j] == 1 || _solution[i][j] == last) {
                    _board[i][j] = new Sq(i, j, _solution[i][j] == 1 ? 1 : last,
            ...
            for (int j = 0; j < _height; j += 1) {
                if (_board[i][j].sequenceNum() != last) {
                    for (Place temp : _board[i][j]._successors) {
                        if (_board[temp.x][temp.y].sequenceNum() != 1) {
                            _board[temp.x][temp.y]._predecessors.
                                    add(_board[i][j].pl);
                        }
                    }
                }
    ...
    /** Initializes a copy of MODEL using model as reference.
     * PARAM @NotNull
     **/
    Model(Model model) {
        ...
        this._allSquares = new ArrayList<>(_width * _height);
        for (int i = 0; i < _width; i += 1) {
            for (int j = 0; j < _height; j += 1) {
                this._board[i][j] = new Sq(model.get(i, j));
        ...
        for (int i = 0; i < _width; i += 1) {
            for (int j = 0; j < _height; j += 1) {
                if (model._board[i][j].successor() != null) {
                    this._board[i][j]._successor =
                            this.get(model._board[i][j].successor().x,
                                    model._board[i][j].successor().y);
                }
                if (model._board[i][j].predecessor() != null) {
                    this._board[i][j]._predecessor =
                            this.get(model._board[i][j].predecessor().x,
                                    model._board[i][j].predecessor().y);
                }
                if (model._board[i][j].head() != null) {
                    this._board[i][j]._head = this.get
                            (model._board[i][j].head().x,
                                    model._board[i][j].head().y);
                }
            }
        }
    }
    ...
    /** Check predecessors connectability.
     * @param s - square
     * @return boolean
     * */
    boolean checkbehind(Sq s) {
    ...
    /** Check successors connectability.
     * @param s - square
     * @return boolean
     * */
    boolean checkahead(Sq s) {
        boolean answer = false;
    ...
    /** Connect all numbered cells with successive
     *  numbers that as yet are
     *  unconnected and are separated
     *  by a queen move.  Returns true iff
     *  any changes were made. */
    boolean autoconnect() {
        boolean answer = false;
        for (int i = 1; i < _width * _height; i += 1) {
            if (_board[solnNumToPlace(i).x][solnNumToPlace(i).y].
                    successor() == null) {
                if (_board[solnNumToPlace(i).x]
                        [solnNumToPlace(i).y].group() == 0) {
                    if (checkahead(_board[solnNumToPlace(i + 1).x]
                            [solnNumToPlace(i + 1).y])) {
                        _board[solnNumToPlace(i).x][solnNumToPlace(i).y].
                                connect(_board[solnNumToPlace(i + 1).x]
                                [solnNumToPlace(i + 1).y]);
    ...
    /** Sets the numbers in this board's squares
     * to the corresponding values in solution from which
    ...
    final class Sq {
        ...
        boolean connectable(Sq s1) {
            ...
            if (Place.dirOf(this.x, this.y, s1.x, s1.y) == this._dir) {
                ...
                if (s1.predecessor() != null || this.successor() != null) {
                    answer = false;
                } else if (s1.successor() == this) {
                    answer = false;
                }

                if ((s1.group() == 0 && this.group() == 0)
                        && this.sequenceNum() != s1.sequenceNum() - 1) {
                    answer = false;
                }
                if (s1.group() != 0 && this.group() != 0
                        && s1.group() == this.group()) {
                    if (s1.sequenceNum() > this.sequenceNum()) {
                        answer = false;
                    }
                }
                if (s1.sequenceNum() == 1) {
                ...
                if (this.sequenceNum() == width() * height()) {
                    answer = false;
                }
        ...
        /** Number all the successors of square s, according to it's number
         * which is realized from a connection with a numbered square.
         * @param s - Square s whose successors are to be numbered.
         */
        void numSuccessors(Sq s) {

            if (s.successor() != null) {
                s._successor._sequenceNum = s.sequenceNum() + 1;
                numSuccessors(s.successor());
            }
        }

        /** Number all the predecessors of square s, according to it's number
         * which is realized from a connection with a numbered square.
         * @param s - Square s whose predecessors are to be numbered.
         */
        void numPredecessors(Sq s) {

            if (s.predecessor() != null) {

                s._predecessor._sequenceNum = s.sequenceNum() - 1;
                numPredecessors(s.predecessor());

            }
        }

        /** Sets the _head fields of all the successors of s,
         * to the head of s.
         * @param s - Square s whose successor head are to be changed.
         * @param mainHead - head of the group.
         */
        void successorHeads(Sq s, Sq mainHead) {
            if (s.successor() != null) {

                s._successor._head = mainHead.head();
                successorHeads(s.successor(), mainHead);
            }
        }

        /** Change the group number of all successors of
         *  square s to zero.
         * @param s - square whose successors number is to be changed.
         */
        void groupZerosuc(Sq s) {
            s._group = 0;
            if (s.successor() != null) {
                groupZerosuc(s.successor());
            }
        }

        /** Change the group number of all predecessors of
         *  square s to zero.
         * @param s - square whose predecessors number is to be changed.
         */
        void groupZeroprev(Sq s) {
            s._group = 0;
            if (s.predecessor() != null) {
                groupZeroprev(s.predecessor());
            }
        }
        ...
        boolean connect(Sq s1) {
            ...
            if (thisgroup == 0 && sgroup != 0) {
                ...
                releaseGroup(sgroup);
                groupZerosuc(s1);
            } else if (sgroup == 0 && thisgroup != 0) {
                this._sequenceNum = s1.sequenceNum() - 1;
                if (this.predecessor() != null) {
                    numPredecessors(this);
                }
                releaseGroup(thisgroup);
                groupZeroprev(this);

            } else if (thisgroup == 0 && sgroup == 0) {
                if (s1.successor() != null) {
                    numSuccessors(s1);
                }
                if (this.predecessor() != null) {
                ...
                int newgroup = joinGroups(thisgroup, sgroup);
                this._head._group = newgroup;
                setSuccessorGroups(this._head, newgroup);
            }

            s1._head = this.head();
            if (s1.successor() != null) {
                successorHeads(s1, this.head());
            }

            return true;
        }

        /** Sets group number of all successors of Square s
         * to newgroup.
         * @param s - Square
         * @param newgroup - new group number
         */
        void setSuccessorGroups(Sq s, int newgroup) {
            if (s.successor() != null && !s._successor.hasFixedNum()) {
                s._successor._group = newgroup;
                setSuccessorGroups(s._successor, newgroup);
            }
        }

        /** Sets group number of all predecessors of Square s
         * to newgroup.
         * @param s - square
         * @param newgroup - new group number
         */
        void setPredecessorgroups(Sq s, int newgroup) {
            if (s.predecessor() != null && !s._predecessor.hasFixedNum()) {
                s._predecessor._group = newgroup;
                setPredecessorgroups(s.predecessor(), newgroup);
            }
        }

        /** Checks all predecessors of square s if they have a
         * fixed sequence number. Returns true iff found.
         * @param s - square
         * @return boolean value
         */
        boolean checkPrevforfixed(Sq s) {
            boolean answer = false;
            if (!s.hasFixedNum()) {
                answer = false;
                if (s.predecessor() != null) {
                    answer = checkPrevforfixed(s.predecessor());
                }
            } else {
                answer = true;
            }
        ...
        /** Checks all successors of square s if they have a
         * fixed sequence number. Returns true iff found.
         * @param s - square
         * @return answer (boolean)
         */
        private boolean checkSucforfixed(Sq s) {
            boolean answer = false;
            if (!s.hasFixedNum()) {
                answer = false;
                if (s.successor() != null) {
                    answer = checkSucforfixed(s.successor());
                }
            } else {
                answer = true;
            }
        ...
        /** Sets sequence number of all predecessors of
         * square s to zero.
         * @param s - square
         */
        private void setPrevzeroseq(Sq s) {
            s._sequenceNum = 0;
            if (s.predecessor() != null) {

                setPrevzeroseq(s.predecessor());
            }
        }

        /** Sets sequence number of all successors of
         * square s to zero.
         * @param s - square
         */
        private void setSuczeroseq(Sq s) {
            s._sequenceNum = 0;
            if (s.successor() != null) {
                if (!s._successor.hasFixedNum()) {

                    setSuczeroseq(s.successor());
                }
            }
        }
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                ...
                if (this.predecessor() == null && next.successor() == null) {
                    ...
                    setSuccessorGroups(next, newgroup);

                }
            } else {

                if (!checkPrevforfixed(this)) {
                    setPrevzeroseq(this);
                    if (this.predecessor() != null) {
                        int newgroup = newGroup();
                        this._group = newgroup;
                        setPredecessorgroups(this, newgroup);
                    } else {
                        this._group = -1;
                    }
                }
                if (!checkSucforfixed(next)) {
                    setSuczeroseq(next);
